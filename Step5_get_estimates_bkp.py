...
def Step5_get_estimates_bkp(subj_list, cfg):
    ...
    for subj in subj_list:
        ...
        for sess in sess_list:
           ...
           for model in cfg['model_list']:
                ...
                # Get atlas and tissue probability maps in dwi space and atlas labels
           ...
           for Delta in Delta_list:   
                 ...
                 bids_strc_reg  = create_bids_structure(subj=subj, sess=sess, datatype='registration', description=cfg['atlas']+'_To_'+f'Delta_{Delta}_fwd', root=data_path, 
                                             folderlevel='derivatives', workingdir=cfg['analysis_foldername'])
                 bids_strc_reg.set_param(base_name='')
                 ...
                 atlas_labels = pd.read_csv(
                     ...
                     names=['IDX', 'R', 'G', 'B', 'A', 'VIS', 'MSH', 'LABEL'], 
                     quotechar='"',)  
                 bids_strc_reg_TPM  = create_bids_structure(subj=subj, sess=1, datatype='registration', description=cfg['atlas_TPM']+'_To_'+f'Delta_{Delta}_fwd', root=cfg['data_path'] , 
                                              folderlevel='derivatives', workingdir=cfg['analysis_foldername'])
                 bids_strc_reg_TPM.set_param(base_name='')
                 TPMs = [bids_strc_reg_TPM.get_path('atlas_TPM_GM_in_dwi.nii.gz'), 
                        bids_strc_reg_TPM.get_path('atlas_TPM_WM_in_dwi.nii.gz'),
                 ...
                 for ROI in ROI_list:
                    ...
                    for pattern in patterns:
                         ...
                         param_masked = parameter_data * 1 * mask_indexes
                         data = param_masked.reshape(param_masked.shape[0]*param_masked.shape[1]*param_masked.shape[2], 1)
                         data = data[~(np.isnan(data).any(axis=1) | (data == 0).any(axis=1) | (data > 100).any(axis=1) | (data < 0).any(axis=1))]
                         
                         # Store the mean of the masked data
                         Data_DTIDKI[Delta_ctr,ROI_ctr, pattern_ctr] = np.nanmean(data) if len(data) > 0 else np.nan
           ...
           ROI_ctr = 0
           for ROI_ctr in range(len(ROI_list)):
               line, =  ax[0].plot(Delta_list, np.squeeze(Data_DTIDKI[:,ROI_ctr,0]),  linestyle='--', marker='o', c=color_list[ROI_ctr])
               ax[1].plot(Delta_list, np.squeeze(Data_DTIDKI[:,ROI_ctr,1]),  linestyle='--', marker='o', c=color_list[ROI_ctr])
               ax[2].plot(Delta_list,np.squeeze(Data_DTIDKI[:,ROI_ctr,2]), linestyle='--', marker='o', c=color_list[ROI_ctr])
           ...
           ax[2].set_xlabel('Diffusion time [ms]')
           plt.legend(handles=line_handles,labels=ROI_list,loc='upper right',fontsize=7)    
           plt.suptitle('DKI')
           plt.rc('font', size=9)
           outfile   = os.path.join(os.path.dirname(os.path.dirname(output_path)),'DKI.png')
           plt.savefig(outfile)
