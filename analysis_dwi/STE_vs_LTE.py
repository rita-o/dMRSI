...
for subj in subj_list:
    ...
    bids_LTE      = create_bids_structure(subj=subj, sess=1, datatype='dwi', root=cfg['data_path'] , 
                 folderlevel='derivatives', workingdir=cfg['analysis_foldername'],description='pwd_avg_Delta_15')
    extract_vols(os.path.join(bids_LTE.get_path(),'pwd_avg_norm.nii.gz'), bids_LTE.get_path('b0.nii.gz'), 0, 1)

    # Get atlas in dwi space and atlas labels
    bids_strc_reg  = create_bids_structure(subj=subj, sess=1, datatype='registration', description='Delta_15_fwd', root=cfg['data_path'] , 
    ...
    #S_S0_LTE  = nib.load(os.path.join(bids_LTE.get_path(),'pwd_avg_norm.nii.gz')).get_fdata()    
    ...
    for sess in sess_list:
            ...
            for ROI in cfg['ROIs']:
                ...
                if sess == 1:
                    ...
                    mask_diff = np.arange(0, 5)
                    filtered_bvals = filtered_bvals[mask_diff]
                    ...
                    filtered_LTE = filtered_LTE[mask_diff]
                ...
                # Set axes
                if k==0:
                    axs[k].set_ylabel(r'$S / S_0$', fontdict={'size': 12, 'weight': 'bold', 'style': 'italic'})  
