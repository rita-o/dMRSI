...
addpath(genpath('Z:\Jessie_DWSdata\FID-A-master')) % adapt if needed
...
for file=1:length(filelist)
   ...
   [out1alb,fsa,phsa]=op_alignAverages_fd_jm_conj(out0alb,4.7+spectralreginf,4.7+spectralregsup,0.5,'y');%5.2,9,0.5,'y');
    ...
    clear fidtot;
    fidb=out1b.fids.'; 
    fidtot(1:1:size(fidb,1),:)=fidb;
    fid2sum=fidtot;
    
    ind=1;
    clear fidmocor;
    for k=1:size(fidtot,1)  
...
for file=1:length(filelist)
    ...
    ftcorr=fftshift(fft(sumfid,[],2),2); 
