...
folderexp   = "Y:\DATA\CSI\9.4T-FIDCSI-Data\20250424_084847_Toi_dMRSI_20250424_RatFem_dMRI_dSPECIAL_dMRSI_1_11_cc";

expnb       = 43; % scan number


%% step 1 - do preprocessing with FIDA
filelist = dir(fullfile(folderexp, "/raw/*ser.mat"));
...
for file=1:length(filelist)
    ...
    out0a=convertNicoStudyFidAformat_isison(char(fullfile(folderexp,'raw')), filelist(file).name);
    out0b=convertNicoStudyFidAformat_isisoff(char(fullfile(folderexp,'raw')), filelist(file).name);
    %apply LB
    dw=out0a.dwelltime;
...
%     out2alb.fids=conj(fids2alb);
    ...
    if dosaveprocessing
        load(char(fullfile(folderexp,'raw',filelist(file).name)));
...
for file=1:length(filelist)
    ...
    plot(real(ftcorr))
    hold on 
    load(char(fullfile(folderexp,'raw',[filelist(file).name(1:end-4) '.mat'])),'study')   
    ...
    plot(real(ftini)); xlim(gca,[2000 3500]);
